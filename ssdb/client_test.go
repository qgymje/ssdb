...
func dial(t *T) *Client {
	client, err := DialTimeout("tcp", "127.0.0.1:8888", 10*time.Second)
	require.Nil(t, err)
	return client
}
...
func TestCmd(t *T) {
	c := dial(t)
	echo := randStr()
	err := c.Cmd("set", echo, echo).Err
	require.Nil(t, err)
	v, _ := c.Cmd("get", echo, echo).Str()
...
func TestPipeline(t *T) {
	...
	for i := 0; i < 10; i++ {
		...
		c.PipeAppend("get", "bar")
		c.PipeAppend("get", "zot")
